...
"""
Inherits the native Oddo module for stock transfers.
"""
class SimpleStockTransferLine(models.Model):
    ...
    def _get_uom(self):
        if self.product_id:
            ...
            raise Exception("Something is not right with the provided parameters for UOM")
    ...
    quantity = fields.Float(string="Quantity",
                            required= True,
                            default=1.0)
    transfer_uom = fields.Many2one('product.uom',
                                   string="Transfer UOM",
                                   store=True,
    ...
    unit_price = fields.Float(string="Unit Price",
                              required= True,
                              default=0.0)
    line_total_price = fields.Float(string="Subtotal",
                                    store=True)
    
    simple_stock_transfer_id = fields.Many2one('simple.stock.transfer',
                                               string="Simple Transfer Reference",
                                               ondelete='cascade',
                                               index=True)
    ...
    def get_quantity_at_location(self,warehouse_stock_location,product_id):
        sql = """select COALESCE( SUM(qty), 0) from stock_quant where 
                  location_id = %d and product_id = %d""" % (warehouse_stock_location, product_id)
...
class SimpleStockTrasfer(models.Model):
    ...
    partner_id = fields.Many2one('res.partner',
                                 ...
                                 required= True)
    source_location_id = fields.Many2one('stock.warehouse',
                                         string="Source Location",
                                         track_visibility='onchange',
                                         required= True)
    destination_location_id = fields.Many2one('stock.warehouse',
                                              string="Destination Location",
                                              track_visibility='onchange',
                                              required= True)
    
    move_type = fields.Selection(selection=[('internal_transfer', 'Internal Transfer'),
    ...
    name = fields.Char(track_visibility='onchange',
                       default=lambda self: self.pool['ir.sequence'].get(self.env.cr, self.env.uid, 'simple_transfer'),
                       required= True,
                       compute='_assign_transfer_code')
    ...
    simple_stock_tranfer_line = fields.One2many('simple.stock.transfer.line',
                                                'simple_stock_transfer_id',
                                                string="Simple Stock Tranfer Line")
    ...
    def _check_change(self):
        ...
        if self.source_location_id.id == self.destination_location_id.id:
            ...
            raise Exception("Source location and destination is the same."
                            "Please check the values again.")
            
    # Check if there is available stocks in the source location
    @api.multi
    def check_simple_stock_moves(self):
        warning = 'You are trying to transfer %s of %s but you only have %s in location %s.'
        if not self.simple_stock_tranfer_line:
            raise osv.except_osv(_('No products specified'),
                                 _(' Please specify some products to transfer.' ) )
            
        for simple_transfer_line in self.simple_stock_tranfer_line:
            current_stock = simple_transfer_line.get_quantity_at_location(self.source_location_id.lot_stock_id.id,
                                                                          simple_transfer_line.product_id.id)
             
            if simple_transfer_line.quantity > current_stock:
                query = """ update simple_stock_transfer set state = 
                          'waiting_for_availability' where id = %s """%self.id
                self._cr.execute(query)
                raise osv.except_osv(_('Not Enough Stock'),
                                     _(warning.format(simple_transfer_line.quantity,
                                                      simple_transfer_line.product_id.name,
                                                      current_stock,
        ...
        for simple_transfer_line in self.simple_stock_tranfer_line:
            if simple_transfer_line.quantity <= 0:
                raise osv.except_osv(_('Quantity Error'),
                                     _('It appears that you have specified a wrong quantity in one of the products.'))
            ...
            for move in  self.pool.get('stock.move').browse(self.env.cr,
                                                            self.env.uid,
                                                            list(id),
                                                            context=None):
                move.action_confirm()
